...
float3 getPixel(float2 coord, float time)
{
    ...
    // color
    //return lerp(getSkyColor(dir), float3(0.5f, 0.5f, 0.9f), pow(smoothstep(0.0, -0.02, dir.y), 0.2));
...
{
    float time = iTime * 0.3 + 0.5f * 0.01;
	
#ifdef AA
    float3 color = float3(0.0, 0.f, 0.f);
    for(int i = -1; i <= 1; i++) {
        for(int j = -1; j <= 1; j++) {
        	float2 uv = input.uv + float2(i,j) / 3.0;
...
//float4 PS(VSOut input) : SV_Target
//{
//    // ShaderToy와 동일하게 fragCoord 계산 (좌표계 보정 포함)
//    float2 fragCoord = float2(input.position.x,
//                              iResolution.y - input.position.y);
//
//    float4 color;
